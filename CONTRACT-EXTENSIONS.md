...
**作用域（如实划清，别当成万能）**：换绑**只支持 checker 绑定**（`counterExample` = `checker:<规格文件>`）
—— 判据演进是它存在的理由。对**文件载体绑定**声明换绑 ⇒ `EFFECT_SUPERSEDE_UNSUPPORTED` 拒写，
合法出路是整条退役（`EFFECT_RETIRE_CANDIDATE`）后用一条新提案重挂。
