...
> **更正（2026-09-21，独立验收 F-2/F-3 修复批）** —— 上面三条里有两处与代码实际行为不一致，逐条改正：
> 1. **落点口径改为"项目落点 ∪ 用户级落点"**（上一条"项目优先、用户级兜底"作废）：作用域文本按**并集**取，
>    同一纪律只念一次，预算按**轮转**分配（每个落点轮流出一条）⇒ 项目规则再多也挤不掉用户级纪律；
>    遥测**逐落点记账**（项目纪律记项目落点、用户级纪律记用户级落点）。
>    为什么必须改：旧口径下"有项目落点就不看用户级"，而根通道在会话接管后会**让路** ⇒
>    **有项目落点的会话永远收不到用户级纪律**（现场实证：一个项目会话的上下文里 `JUDGEMENT-*` 三条
>    静默消失，只剩 `CAT-*`）。**静默缺失比重复严重**。
> 2. **"所有会话接管才让路"这句此前只是文字** —— 代码当时是"**任一会话**接管就让路" ⇒ 只要一个会话
>    接管成功，**任何未接管会话**（注册失败 / 新会话首轮窗口）就**零提醒且无兜底**（独立验收给出可复现
>    红态样本）。现代码已改回"**全部在册会话都有通道才让路**"（在册列表取自 `ctx.agents.roots()`，
>    取不到就**不让路**），并给注册失败加了**有界重试**（`SCOPED_MAX_ATTEMPTS=3`）。
>    已知代价（如实记账）：启动竞态窗口内根通道会先投一次，该会话随后又收到自己那份 ⇒ 同一份提醒
>    重复一次，定级 nit —— **宁重复，不静默**。
...
## 计划工件对账（`plan-artifact` 检查器 + `rk-plan`，2026-09-23 · **V1 observe**）

> 「动手前先有计划」这条纪律的机械切片。**先说为什么不是"直接加一条检查"**（按规则 53 先量后写，数据否掉了两个直觉形态）：
> ① 「改代码的提交前 24h 必须有 `plan/design` 工件」⇒ 本仓近 60 提交 **53/53 = 100% 命中**（该仓历史上 `plan/design` 改动 0 条）
> ⇒ 上线第一天判红所有代码提交；② 「提交正文必须含要点」⇒ **18/34 = 53%**，超止损线且测的是关键词。
> 选定形态：**把计划工件落成台账行**，门禁按"行 ↔ 改动"对账（与「受保护文件改了没留证 ⇒ 拒提交」同一机制，只换对象）。

**怎么用（被治理仓侧，两步）**：

```bash
# 1) 在落点 config.json 里声明"哪些算代码类 + 窗口多长"（**口径由你声明**，插件不硬编扩展名）
#    <项目>/.dsh-ai/rulekeeper/config.json
#    { "planScope": { "codeGlobs": ["**/*.mjs", "**/*.go"], "windowHours": 24 } }

# 2) 动手前声明计划（写一条台账行）
node bin/rk-plan.mjs declare --landing <项目>/.dsh-ai/rulekeeper \
  --why "为什么要做（不是做了什么）" --criteria "判据" --rollback "回滚" --red "反向红/验证" --scope "**/*.mjs"

# 3) 对账（默认 observe：只记审计、不阻断）
RULEKEEPER_PLAN_BASE=HEAD~1 RULEKEEPER_PLAN_HEAD=HEAD node scripts/checkers/plan-artifact.mjs
#    加 RULEKEEPER_PLAN_MODE=armed 才以非 0 退出（**升级到阻断需另一次决策 + 真仓命中率数据**）
```

**三态分明**：`0` = 覆盖（pass）／`1` = 命中（**仅 armed 档**）／`2` = 不适用
（没声明 `planScope`／改动里没有代码类文件／台账里 0 条计划行 —— **"没得判"绝不等于"通过"**）。

**诚实边界（写在输出里，规则 43）**：它判的是"计划工件存不存在、时点对不对"，**判不了"你是不是真先想过"**；
`why/criteria/rollback` 是**声明**不是签名（能写台账的人也能把话说得漂亮）；钩子只在**提交时**跑 ⇒
**抓不到"你已经开始乱改了"**（要动手前拦，只能靠宿主在写操作前拦截）；**observe 档不阻断任何东西**。

**为什么不挂绑定**：`misreport-surface` 要求每条 checker 绑定两态成对，而本检查器的结论由**落点配置 + git 历史**决定，
样本目录里表达不出来 ⇒ 硬造 spec 会产出"看起来有判别力、实际没核过"的假绑定。判别力**由
`test/plan-artifact.test.mjs` 的 6 条用例（含两处反向红）证明**。
